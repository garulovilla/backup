...
const add = async (configFile, fileOrFolder, options) => {
  ...
  // Add new object
  config.backup.push({
    path: util.resolvePath(fileOrFolder),
    ...answers
  })
...
const run = async (configFile) => {
  ...
  for (const element of config.backup) {
    ...
    // Check if current element needs to be backup
    if (!element.active) {
      continue
    }
    ...
    const existFileOrFolder = await util.existFileOrFolder(absolutePath)
