...
const readConfigFile = async (localPath) => {
  try {
    ...
    for (const element of config.backup) {
      ...
      // If temp property doesn't exist, then set default value
      if (!('temp' in element)) {
        element.temp = false
      }
    }
...
const resolveRenamePattern = (element, base) => {
  ...
  // Replace /s with timestamp yyyyMMddhhmmss
...
const deleteOldBackupFilesOrFolders = async (element, from, to) => {
  ...
  // Replace patterns in rename with regex
  let rename = element.rename
...
const backup7z = async (element, to) => {
  ...
  const filterFilesToCopy = async (src, dest) => {
    // Copy all but not symbolic links
  ...
  // Copy to temporal folder first?
  if (element.temp) {
    // Create temp folder
    try {
      tempFolder = await fsp.mkdtemp(path.join(os.tmpdir(), 'bak-'))
    } catch (err) {
      log.error(err);
    }

    log.info(`Temp: ${log.path(tempFolder)}`)

    // Copy from to temp folder
    for (const fromElement of from) {
      const fromElementBase = path.parse(fromElement).base
      const pathToTempFolder = tempFolder + `\\${fromElementBase}`
      try {
        await fse.copy(fromElement, pathToTempFolder, { filter: filterFilesToCopy })
      } catch (e) {
        log.error(e)
      }
    }

    fromTemp.push(`${tempFolder}\\*.*`)
  } else {
    fromTemp = from
  }

  // Create 7z file
  const stream = seven.add(absolutePathTo, fromTemp, {
    $bin: pathTo7zip,
    recursive: true
  })
  ...
  if (element.temp) {
    try {
      await fsp.rmdir(tempFolder, { recursive: true })
    } catch (e) {
      log.error(e)
    }
  }
